...
let handler = async (m, { conn, command, text, usedPrefix }) => {
  ...
  if (!vid) return m.reply(`⚠️ *No se encontraron resultados.*`)
  ...
  if (!json.status) return m.reply(`❌ *Error al procesar la descarga.*`)
  ...
  cap += `⚙️ *Box Bot MD • Descargando...* 🌀`

  await conn.sendMessage(m.chat, { image: { url: vid.thumbnail }, caption: cap }, { quoted: m })
  await conn.sendMessage(m.chat, { [isVideo ? 'video' : 'audio']: { url: json.data.dl }, mimetype: isVideo ? 'video/mp4' : 'audio/mpeg' }, { quoted: m })
}
